class BinaryTreeAsList:
    ...
    def inorderTraversal(self):
        ...
        Returns a list of nodes in inorder traversal.
        """
        return self.__inorderTraversalRec(self.__root)

    def __inorderTraversalRec(self, node):

        result = []
        if node.left == None and node.right == None:
            return [node]
        if node.left != None:
            result.extend(self.__inorderTraversalRec(node.left))
        result.append(node)
        if node.right != None:
            result.extend(self.__inorderTraversalRec(node.right))
        return result
...
if __name__ == "__main__":
    ...
    for node in binTree.inorderTraversal():
        print(node.value)