...
class Graph:
    ...
    def dfs(self, startNodeName, searchValue):
        """!
        Searches in the graph with depth first search.
        Returns True, if the value was found, otherwise false.
        """
        startIndex = [x.index for x in self.nodes if x.name == startNodeName][0]
        adjMatrix = self.getAdjacentMatrix()
        nodeMapping = {x.index: x.name for x in self.nodes}

        return self.__dfsRec(startIndex, searchValue, adjMatrix, nodeMapping)

    def __dfsRec(self, nodeIndex, searchValue, adjMatrix, nodeMapping):
        
        if nodeMapping[nodeIndex] == searchValue:
            return True
        else:
            for i in range(len(adjMatrix)):
                if adjMatrix[nodeIndex][i] != 0.:
                    found = self.__dfsRec(i, searchValue, adjMatrix, nodeMapping)
                    if found:
                        return True
        return False
