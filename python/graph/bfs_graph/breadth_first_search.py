...
class Graph:
    ...
    def bfs(self, startNodeName, searchValue):
        """!
        Performs a breadth first search on the graph.
        Returns True, if the searchValue was found, otherwise False.
        """
        startIndex = [x.index for x in self.nodes if startNodeName == x.name][0]
        queue = [startIndex]
        visitedVertices = []

        indexNameMapping = {x.index: x.name for x in self.nodes}
        ...
        while len(queue) > 0:
            ...
            currentNode = queue[0]
            queue = queue[1:]
            visitedVertices.append(currentNode)

            # value found            
            if indexNameMapping[currentNode] == searchValue:
                return True
            # proceed with the children
            else:
                for i in range(len(adjMatrix)):
                    if adjMatrix[currentNode][i] != 0. and \
                        not i in visitedVertices:
                        queue.append(i)
        return False
